...
export class WriteBatch {
  ...
  /**
   * Commits all queued operations sequentially.
   * This is NOT atomic — if one operation fails, previous operations
   * are not rolled back. Use globio.doc.transaction() for atomic operations.
   */
    ...
    for (const operation of this.operations) {
      ...
      if (operation.type === 'set') {
        result = await this.client.request({
          service: 'doc',
          path: `/${operation.collection}/${operation.docId}`,
          method: 'PUT',
          body: operation.data,
          auth: true,
        });
        ...
        result = await this.client.request({
          service: 'doc',
          path: `/${operation.collection}/auto`,
          method: 'PUT',
          body: operation.data,
          auth: true,
        });
