...
// Indexes are created automatically the first time
// you query a field. No manual index setup required
// for single-field queries.

// For explicit index management:
await globio.doc.createIndex('players', {
  field_path: 'region',
  index_type: 'asc',
});
await globio.doc.listIndexes('players');
...
## GlobalCode — Edge Functions and GC Hooks

GlobalCode gives your game serverless logic at Cloudflare's
edge with full access to every Globio service.

Two products. One API.

### GC Edge Functions

HTTP-triggered functions invoked directly by your game client.
Write server-side game logic without running your own server.
```typescript
// create an edge function (admin/server-side)
await globio.code.createFunction({
  name: 'Matchmaking',
  slug: 'matchmaking',
  type: 'function',
  code: `
    async function handler(input, globio) {
      const { userId, rating, region } = input;

      // query GlobalDoc via injected globio object
      const players = await globio.doc.query('players', {
        where: [
          { field: 'region', op: '==', value: region },
          { field: 'rating', op: '>=', value: rating - 100 },
        ],
        limit: 5
      });

      if (!players.data.length) return { matched: false };

      // create a sync room for the match
      const room = await globio.sync.createRoom({
        name: 'ranked-match'
      });

      return { matched: true, roomId: room.data.id };
    }
  `
});

// invoke from game client
const result = await globio.code.invoke('matchmaking', {
  userId: 'user_123',
  rating: 1450,
  region: 'africa'
});

console.log(result.data.result); // { matched: true, roomId: '...' }
```

| Method | Description |
|---|---|
| createFunction() | Create an edge function |
| listFunctions() | List edge functions only |
| getFunction() | Get function with code |
| updateFunction() | Update function |
| deleteFunction() | Delete function |
| toggleFunction() | Enable or disable |
| invoke() | Invoke a function |
| getInvocations() | Invocation history |

### GC Hooks

Event-driven functions that fire automatically when things
happen in your Globio project. No polling. No webhooks to
configure. Pure reactive game logic.
```typescript
// fires every time a player signs up
await globio.code.createHook({
  name: 'Init Player',
  slug: 'init-player',
  trigger: 'id.onSignup',
  code: `
    async function handler({ userId, email }, globio) {
      ...
      await globio.signal.sendToUser(userId, {
        title: 'Welcome!',
        body: 'Your adventure begins. Here are 100 coins.',
        priority: 'high'
      });
    }
  `
...
// fires every time a purchase completes
await globio.code.createHook({
  name: 'Grant Achievement on Purchase',
  slug: 'purchase-achievement',
  trigger: 'mart.onPurchase',
  code: `
    async function handler({ userId, sku }, globio) {
      const inventory = await globio.mart.getInventory();
      const totalItems = inventory.data.length;

      if (totalItems >= 10) {
        await globio.signal.sendToUser(userId, {
          title: 'Achievement unlocked!',
          body: 'Collector — own 10 items',
        });
      }
    }
  `
...
| Method | Description |
|---|---|
| createHook() | Create a GC Hook |
| listHooks() | List hooks only |
| updateHook() | Update hook code or trigger |
| deleteHook() | Delete hook |
| toggleHook() | Enable or disable |
| getHookInvocations() | Invocation history |

Hooks cannot be invoked manually.
They fire automatically when the trigger event occurs.
...
- Network access: disabled (use the injected globio object)
- Timeout: 5 seconds (edge functions), 10 seconds (hooks)
- Handler signature: async function handler(input, globio)

### Managing Edge Functions
```typescript
// list edge functions only
const fns = await globio.code.listFunctions();

// get function with code
const fn = await globio.code.getFunction('matchmaking');

// update code
await globio.code.updateFunction('matchmaking', {
  code: '...'
});

// toggle on/off
await globio.code.toggleFunction('matchmaking', false);

// delete
await globio.code.deleteFunction('matchmaking');

// invocation history
const history = await globio.code.getInvocations('matchmaking');
history.data.forEach(inv => {
  console.log(inv.duration_ms, inv.success);
});
```
...
```typescript
// list hooks only
const hooks = await globio.code.listHooks();

await globio.code.updateHook('init-player', {
  trigger: 'id.onSignin',
});

await globio.code.toggleHook('init-player', false);
await globio.code.deleteHook('init-player');

const hookHistory = await globio.code.getHookInvocations('init-player');
```

Breaking change:
`listFunctions()` now returns edge functions only.
Use `listHooks()` to list hooks.
